...
class Loss(nn.Module):
    def __init__(self, num_hard=0, class_loss='BCELoss', average=True):
        ...
        loss_dict = nn.ModuleDict(
            {'BCELoss': nn.BCELoss(),
             ...
             'MSELoss': nn.MSELoss(reduction='elementwise_mean' if average else 'sum'),
    ...
    def forward(self, output, labels, train=True):
        ...
        if isinstance(self.classify_loss, nn.BCELoss):
        ...
        if len(pos_output) > 0:
            return [loss, classify_loss_data] +[0]+ [pos_correct, pos_total, neg_correct, neg_total], pos_prob, pos_labels[:,0], neg_prob, neg_labels+1
        else:
            return [loss, classify_loss_data] +[0]+ [pos_correct, pos_total, neg_correct, neg_total], -100, -100, neg_prob, neg_labels+1
...
class Loss_recon(nn.Module):
    def __init__(self, num_hard=0, class_loss='MarginLoss', recon_loss_scale=1e-6, average=True):
        ...
        self.reconstruction_loss = nn.MSELoss(reduction='elementwise_mean' if average else 'sum')
    ...
    def forward(self, output, labels, images, reconstructions, train=True):
        ...
        neg_total = len(neg_prob)
        if len(pos_output) > 0:
            return [loss, classify_loss_data] + regress_losses_data + [pos_correct, pos_total, neg_correct, neg_total] + [reconstruction_loss_data], pos_prob, pos_labels[:,0], neg_prob, neg_labels+1
        else:
            return [loss, classify_loss_data] + regress_losses_data + [pos_correct, pos_total, neg_correct, neg_total] + [reconstruction_loss_data], -100, -100, neg_prob, neg_labels+1
...
class CapsuleLoss(nn.Module):
    def __init__(self, loss_lambda=0.5, recon_loss_scale=5e-4, size_average=False):
        ...
        self.reconstruction_loss = nn.MSELoss(reduction='elementwise_mean' if size_average else 'sum')
        self.recon_loss_scale = recon_loss_scale
