'''
Convolution Capsule U-Net without dilation
use dout2_2 and coord to get confidence and location
...
from ConvCapsule_layers_3d import DeconvCapsuleLayer as DeConvCap
...
class CapNet(nn.Module):
    def __init__(self, output_feature=False):
        ...
        self.convert_to_capsule = ConvertToCapsule(num_cap=1)
        self.cap_len = CapToLength(keepdim=False)
        self.conv0_1 = nn.Conv3d(1, 64, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn0_1 = nn.BatchNorm3d(64)
        self.conv0_2 = ConvCapBlock(1, 64, 8, 8, 3, 1, 1, 1, 1, norm)  # primary capsule
        self.conv1_1 = ConvCapBlock(8, 8, 4, 16, 3, 2, 1, 1, 3, norm)
        self.conv1_2 = ConvCapBlock(4, 16, 8, 16, 3, 1, 1, 1, 3, norm)
        self.conv2_1 = ConvCapBlock(8, 16, 4, 32, 3, 2, 1, 1, 3, norm)
        self.conv2_2 = ConvCapBlock(4, 32, 8, 32, 3, 1, 1, 1, 3, norm)
    ...
    def forward(self, x, coord):
        ...
        out0_1 = F.relu(self.bn0_1(self.conv0_1(x)))
        if self.debug: print(f'out0_1={out0_1.size()}')

        out0_2 = self.conv0_2(self.convert_to_capsule(out0_1))
        if self.debug: print(f'out0_2={out0_2.size()}')

        out1_1 = self.conv1_1(out0_2)
        if self.debug: print(f'out1_1={out1_1.size()}')

        out1_2 = self.conv1_2(out1_1)
        if self.debug: print(f'out1_2={out1_2.size()}')

        out2_1 = self.conv2_1(out1_2)
        if self.debug: print(f'out2_1={out2_1.size()}')

        out2_2 = self.conv2_2(out2_1)
        if self.debug: print(f'out2_2={out2_2.size()}')
        ...
        coord = self.convert_to_capsule(coord).repeat(1, 1, 1, 1, n_cap, 1)
        ...
        # assert out[:,:,:,:,:,0] >= 0.5
        # sig = nn.Sigmoid()
        # assert (sig(out.view(-1,5)[:,0]) >= 0.5).sum() == 1*12*12*12*3
        ...
        del out0_1, out1_1, out2_1
        del out0_2, out1_2, out2_2
        del comb, class_cap, class_len, loc_cap
...
def get_model(output_feature=False):
    ...
    loss = Loss_recon(config['num_hard'], recon_loss_scale=0.0, class_loss='FocalMarginLoss')
