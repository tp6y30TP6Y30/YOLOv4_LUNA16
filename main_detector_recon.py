...
def train(data_loader, net, criterion, epoch, optimizer, lr_adjuster):
    ...
    for i, (input, target, coord) in enumerate(pbar):
        ...
        # Compute output
        output, recon = net(input, coord)
        loss = criterion(output, target, input, recon, train=True)
...
def validate(data_loader, net, criterion, epoch, save_dir):
    ...
    with torch.no_grad():
        ...
        for i, (input, target, coord) in enumerate(pbar):
            ...
            output, recon = net(input, coord)
            loss = criterion(output, target, input, recon, train=False)
            loss[0] = loss[0].item()
            metrics.append(loss)
        ...
        with torch.no_grad():
            for i in range(len(splitlist)-1):
                ...
                if isfeat:
                    ...
                    output, recon = net(input, inputcoord)
