...
def main(bbox_path, preprocess_path, lunaseg_path, save_file):
    ...
    for i in tqdm(range(5), ncols = 70):
        ...
        with Path('test_0222_%s/LUNA_test.json' %str(i+1)).open('rt', encoding='utf-8') as fp:
            idcs = json.load(fp)
        for x in range(len(idcs)):
            pbb = np.load('%s%s/bbox/%s_pbb.npy' %(bbox_path, str(i+1), idcs[x]), mmap_mode='r')
            pbb = nms(pbb, 0.01)
            ...
            for nk in range(pos.shape[0]): 
                # rowlist.append([idcs[x], pos[nk, 2], pos[nk, 1], pos[nk, 0], 1/(1+np.exp(-pbb[nk,0]))])
...
if __name__=='__main__':
    main(bbox_path='./results/YOLOv4_testcross', preprocess_path='./data/preprocess/all/', lunaseg_path='./data/LUNA16/seg-lungs-LUNA16/', save_file='./results/YOLOv4_80_all.csv')