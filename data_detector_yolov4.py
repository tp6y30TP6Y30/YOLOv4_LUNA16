    ...
    def __getitem__(self, idx, split=None):
        ...
        if self.phase == 'train' or self.phase == 'val':
            if idx >= len(self.bboxes):
                isRandom = True
                idx = random.randint(0, len(self.bboxes) - 1)
        ...
        if self.phase == 'train' or self.phase == 'val':
            ...
            return torch.from_numpy(sample), torch.from_numpy(label[0]), torch.from_numpy(label[1]), torch.from_numpy(label[2]), torch.from_numpy(coord[0]), torch.from_numpy(coord[1]), torch.from_numpy(coord[2])
            ...
            for s in self.stride:
                ...
                pz = int(np.ceil(float(nz) / s)) * s
                ph = int(np.ceil(float(nh) / s)) * s
                pw = int(np.ceil(float(nw) / s)) * s
                imgs = np.pad(imgs, [[0,0], [0, pz - nz], [0, ph - nh], [0, pw - nw]], 'constant', constant_values=self.pad_value)
                xx,yy,zz = np.meshgrid(np.linspace(-0.5,0.5,imgs.shape[1]//s),
                                       np.linspace(-0.5,0.5,imgs.shape[2]//s),
                                       np.linspace(-0.5,0.5,imgs.shape[3]//s), indexing='ij')
                coord = np.concatenate([xx[np.newaxis,...], yy[np.newaxis,...],zz[np.newaxis,:]],0).astype('float32')
                imgs, nzhw = self.split_comber.split(imgs)
                coord2, nzhw2 = self.split_comber.split(coord,
                                                        side_len=self.split_comber.side_len/s,
                                                        max_stride=self.split_comber.max_stride/s,
                                                        margin=self.split_comber.margin/s)
...
class Crop(object):
    def __init__(self, config):
        ...
        self.stride = config['stride']  # [4, 8, 16]
    ...
    def __call__(self, imgs, target, bboxes, isScale=False, isRand=False):
        ...
        for stride in self.stride:
            xx, yy, zz = np.meshgrid(np.linspace(normstart[0], normstart[0] + normsize[0], self.crop_size[0] // stride),
                                     ...
                                     indexing='ij')
            coord_original = np.concatenate([xx[np.newaxis, ...], yy[np.newaxis, ...], zz[np.newaxis, :]], 0).astype('float32')
            coord.append(coord_original)
...
class LabelMapping(object):
    def __init__(self, config, phase):
        self.stride = np.array(config['stride'])  # [4, 8, 16]
    ...
    def __call__(self, input_size, target, bboxes, filename):
        ...
        for stride in self.stride:
            ...
            label_original = -1 * np.ones(output_size + [len(anchors), 5], np.float32)  #(24, 24, 24, #anchor, 5)
            ...
            if self.phase == 'train' and self.num_neg > 0:
                ...
                label_original[:, :, :, :, 0] = 0
                label_original[neg_z, neg_h, neg_w, neg_a, 0] = -1
            ...
            label_original[pos[0], pos[1], pos[2], pos[3], :] = [1, dz, dh, dw, dd]
            label.append(label_original)
        return label
...
def augment(sample, target, bboxes, coord, ifflip=True, ifrotate=True, ifswap=True):
    ...
    if ifflip:
        ...
        coord = [np.ascontiguousarray(c[:,::flipid[0],::flipid[1],::flipid[2]]) for c in coord]
